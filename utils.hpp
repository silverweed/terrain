...
{
	return stream << "(" << rect.left << ", " << rect.top << " x " << rect.width << ", " << rect.height << ")";
}
...
struct ColorHash {
	...
	{
		return (c.r << sizeof(uint8_t) * 3) + (c.g << sizeof(uint8_t) * 2) + (c.b << sizeof(uint8_t) * 1) + c.a;
	}
};
...
template <typename T>
constexpr float sqrlength(sf::Vector2<T> v)
{
